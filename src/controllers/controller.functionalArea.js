...
export const updateFunctionalArea = async (req, res, next) => {
  try {
    const { id } = req.params;
    const updatedFunctionalArea = await FunctionalArea.findByIdAndUpdate(id, req.body, {
      new: true,
    });
...
export const deleteFunctionalArea = async (req, res, next) => {
  try {
    const deletedFunctionalArea = await FunctionalArea.findByIdAndDelete(req.params.id);
