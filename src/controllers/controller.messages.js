...
const replacePlaceholders = (templateText, customer) => {
  ...
  Object.keys(customer.toObject ? customer.toObject() : customer).forEach(
    (k) => {
...
// Helper: fetch target customers
const fetchTargetCustomers = async ({
  customerIds = [],
  sendToAll = false,
}) => {
  if (sendToAll) return await Customer.find();
 ...
 * body: { templateId, customerIds?, sendToAll? }
 */
...
export const sendWhatsAppByTemplate = async (req, res, next) => {
  try {
    ...
    for (const c of customers) {
      try {
        const phone = c.ContactNumber || c.Contact || "";
        if (!phone) {
          results.push({
            id: c._id,
            name: c.customerName,
