...
export const updateContactCampaign = async (req, res, next) => {
  try {
    const { id } = req.params;
    const updatedContactCampaign = await ContactCampaign.findByIdAndUpdate(id, req.body, {
      new: true,
    });
...
export const deleteContactCampaign = async (req, res, next) => {
  try {
    const deletedContactCampaign = await ContactCampaign.findByIdAndDelete(req.params.id);
